...
void SPI3_IRQHandler(void)
{
  if(__HAL_SPI_GET_FLAG(&SPI3_Handler,SPI_FLAG_RXNE) != RESET)
  {
     ...
     { 
       SPI3RX_COUNT=0;
       DEBUG_USART_Handle_Transmit((uint8_t *)SPI3RX, 100);
     }
  }
}
...
void SPI3_Init(void)
{
   uint16_t itpos = 0;
    ...
    SPI3_Handler.Instance=RF_SPI3;                         //SP3
     HAL_SPI_DeInit(&SPI3_Handler);
     SPI3_Handler.Instance=RF_SPI3; 
    ...
    itpos = 0x01 << (SPI_IT_RXNE & 0x0F);  
    RF_SPI3->SR = (uint16_t)~itpos;

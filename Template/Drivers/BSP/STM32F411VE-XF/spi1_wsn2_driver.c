...
void SPI1_Init(void)
{
    SPI1_Handler.Instance=RF_SPI1;                         //SPI1
...
{
    if(hspi->Instance==RF_SPI1)
    {
    GPIO_InitTypeDef GPIO_Initure;
    
    SPI1_SCK_GPIO_CLK_ENABLE();       //使能GPIOF时钟
    SPI1_CLK_ENABLE();        //使能SPI1时钟
    
    //PE2,5,6
    GPIO_Initure.Pin=SPI1_SCK_PIN|SPI1_MISO_PIN|SPI1_MOSI_PIN;
    GPIO_Initure.Mode=GPIO_MODE_AF_PP;              //复用推挽输出
    GPIO_Initure.Pull=GPIO_PULLUP;                  //上拉
    GPIO_Initure.Speed=GPIO_SPEED_FAST;             //快速            
    GPIO_Initure.Alternate=SPI1_MISO_MOSI_AF;           //复用为SPI1

 ...
 u8 usmart_sys_cmd_exe(u8 *str)
{
	...
	{					   
		...
		case 1://帮助指令
			...
			debug_printf("    USMART一个灵巧的串口调试互交组件,通过");
...
/**
  ...
  * @details usmart扫描函数
 */
void usmart_scan( u8 * buffer, u16 size )
{
	  u8 sta,len;  			   
		*(buffer+size)='\0';	//在末尾加入结束符. 
		sta=usmart_dev.cmd_rec(buffer);//得到函数各个信息
		if(sta==0)usmart_dev.exe();	//执行函数 
		else 
		{  
			len=usmart_sys_cmd_exe(buffer);
